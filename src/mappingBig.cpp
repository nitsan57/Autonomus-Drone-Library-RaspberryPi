...
int main(int argc, char** argv)
{
	...
	for (const auto & entry : fs::directory_iterator(path_to_dir)) {
		...
		if (im_path.find(".jpg") != -1 && pos != -1 && image_data[0] != 'p') {
			...
			imwrite("C:\\Users\\nitsa\\Documents\\Study\\EngProject\\CE-4th-year-project\\getting_object_cord\\x64\\Release\\images\\pred" + std::to_string(i) + ".jpg", detection);
			myfile.close();
			if (image.empty())
			{
